...
# Подключаемся к сессии vk_api
vk_session = vk_api.VkApi(token=cfg.vk_bot.token)
vk = vk_session.get_api()  # Подключаемся к интерфейсу api
upload = vk_api.VkUpload(vk)  # Создаём сеанс для загрузки фото

longpoll = VkBotLongPoll(vk_session, cfg.vk_bot.group_id,
                         wait=25)  # Объект опроса сервера
